    ...
    <tr>
        <td width="100%" valign="top">
            <table width="100%" cellpadding=0 cellspacing=0 border=0 align="center">
                <tr>
                    <%
                        if (intCriticalTasks > 0)
                        {
                    %>
                    <td class="DarkBlueBandStyleText14" align="left">&nbsp;List of Critical Tasks (<%= intCriticalTasks %>
                        )
                    </td>
                    <%
                    }
                    ...
                    {
                    %>
                    <td class="DarkBlueBandStyleText14" align="left">&nbsp;List of Critical Tasks</td>
                    <%
                        }
                    %>
                </tr>
            ...
            <table width="100%" cellpadding=2 cellspacing=2 border=0>
                ...
                <%
                    ...
                    int intSerialNo = 0;
                    for (int iCount = 0; iCount < arlCriticalTasksList.size(); iCount += 8)
                    {
                        ...
                        strTicketETA = arlCriticalTasksList.get(iCount + 5);
                        if (strTicketETA != null && strTicketETA.length() > 0)
                        {
                            ...
                            {
                                if (arlCriticalTasksList.get(iCount).equalsIgnoreCase((String) arlOpenTicketsETAHistory.get(intETACount)) &&
                                        arlCriticalTasksList.get(iCount + 1).equalsIgnoreCase((String) arlOpenTicketsETAHistory.get(intETACount + 1)))
                                {
                        ...
                        if (arlCriticalTasksList.get(iCount + 5) != null && arlCriticalTasksList.get(iCount + 5).length() > 0)
                        {
                            ...
                            Date ETADate = ETA_DATE_FORMAT.parse(arlCriticalTasksList.get(iCount + 5));
                ...
                <tr class="<%= strBackGround %>">
                    ...
                    <td class="txtgrey"><a
                            href="JavaScript:openJIRAWindow('<%= AppConstants.JIRAURL + arlCriticalTasksList.get(iCount + 1) %>', '<%= arlCriticalTasksList.get(iCount + 1) %>')"
                            class="link"><%= arlCriticalTasksList.get(iCount + 1) %>
                    </a>
                    </td>
                    <td class="txtgrey"><a
                            href="JavaScript:openJIRAWindow('<%= AppConstants.JIRAURL + arlCriticalTasksList.get(iCount + 1) %>', '<%= arlCriticalTasksList.get(iCount + 1) %>')"
                            class="link"><%= arlCriticalTasksList.get(iCount + 2) %>
                    </a>
                    </td>
                    <td class="txtgrey"><a
                            href="JavaScript:viewTicketDetails('<%= arlCriticalTasksList.get(iCount) %>','<%= arlCriticalTasksList.get(iCount + 1) %>')"
                            title="Ticket Details"><%= arlCriticalTasksList.get(iCount + 3) %>
                    </a>
                    </td>
                    <td class="txtgrey"><%= arlCriticalTasksList.get(iCount + 4) %>
                    </td>
                    <td class="txtgrey"><%= arlCriticalTasksList.get(iCount + 7) %>
                    </td>
                    <td class="txtgrey" <%= sbETAHistory %>><%= strTicketETA %>
                    </td>
                    <td class="txtgrey" align="center"><%= arlCriticalTasksList.get(iCount + 6) %>
                    </td>
                </tr>
                <%
                        intCounter++;
