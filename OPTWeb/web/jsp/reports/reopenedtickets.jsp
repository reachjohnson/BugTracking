...
<%
    ArrayList<String> arlReopenedTicketList = (ArrayList) request.getAttribute("ReopenedTicketList");
    ...
    int intBugsCount = 0;
    int intTotalCount = 0;
    if (arlReopenedTicketList != null && !arlReopenedTicketList.isEmpty())
    {
        for (int iCount = 0; iCount < arlReopenedTicketList.size(); iCount += 12)
        {
            if (arlReopenedTicketList.get(iCount + 3).equalsIgnoreCase(AppConstants.TASK))
            {
                intTaskCount++;
            }
        }
        for (int iCount = 0; iCount < arlReopenedTicketList.size(); iCount += 12)
        {
            if (arlReopenedTicketList.get(iCount + 3).equalsIgnoreCase(AppConstants.BUG))
            {
                intBugsCount++;
            }
        }
...
<head>
    ...
    <script type="text/javascript" language="JavaScript">
        ...
        function reopenedTicketList() {
            if (document.OPTForm.formsubmit.value == "submitted") {
                return;
            }
            else {
                if (isEmpty(document.OPTForm.ReopenedTicketFromDate.value)) {
                    OPTDialog("Enter From Date", document.OPTForm.ReopenedTicketFromDate);
                    return;
                }
                if (isEmpty(document.OPTForm.ReopenedTicketToDate.value)) {
                    OPTDialog("Enter To Date", document.OPTForm.ReopenedTicketToDate);
                    return;
                }
                if (!compareTwoDates(document.OPTForm.ReopenedTicketFromDate.value, SystemDate, "DD/MM/YYYY", "GREATER")) {
                    OPTDialog("From date should not be greater than current date", document.OPTForm.ReopenedTicketFromDate);
                    return;
                }
                if (!compareTwoDates(RestrictedFromDate, document.OPTForm.ReopenedTicketFromDate.value, "DD/MM/YYYY", "GREATER")) {
                    OPTDialog(FromDateMessage, document.OPTForm.ReopenedTicketFromDate);
                    return;
                }
                if (!compareTwoDates(document.OPTForm.ReopenedTicketToDate.value, SystemDate, "DD/MM/YYYY", "GREATER")) {
                    OPTDialog("To date should not be greater than current date", document.OPTForm.ReopenedTicketToDate);
                    return;
                }
                if (!compareTwoDates(document.OPTForm.ReopenedTicketFromDate.value, document.OPTForm.ReopenedTicketToDate.value, "DD/MM/YYYY", "GREATER")) {
                    OPTDialog("From date should not be greater than To Date", document.OPTForm.ReopenedTicketFromDate);
                    return;
                }
...
<body onload="document.OPTForm.ReopenedTicketFromDate.focus()" onFocus="FocusModalWin()" onClick="FocusModalWin()"
        ...
        <tr>
            <td width="100%" valign="top">
                <table width="100%" align="left">
                    <tr>
                        <td width="100%" class="DarkBlueBandStyleText14">&nbsp;Input Filters to View Reopened Tickets
                        </td>
                    </tr>
                </table>
                                    ...
                                    <td width="25%" class="txtgrey" valign="middle">
                                        <table width="100%" cellpadding=2 cellspacing=1 border=0 align="center">
                                            <tr class="LightBandStyle">
                                                ...
                                                <td width="75%">
                                                    <input type="text" name="ReopenedTicketFromDate" size="10" maxlength="20" class="txtgrey"
                                                           onblur="checkValidDateForObject(this)" value="<%= strFromDate %>"
                                                           onkeypress="CTtrackEnterKey(event)">
                                                    <a href="javascript:showCal('Calendar26','dd/mm/yyyy');"><img src="images/cal.gif" width="20"
                                                                                                                  height="13" border="0"
                                    ...
                                    <td width="25%" class="txtgrey" valign="middle">
                                        <table width="100%" cellpadding=2 cellspacing=1 border=0 align="center">
                                            <tr class="LightBandStyle">
                                                ...
                                                <td width="75%">
                                                    <input type="text" name="ReopenedTicketToDate" size="10" maxlength="20" class="txtgrey"
                                                           onblur="checkValidDateForObject(this)" value="<%= strToDate %>"
                                                           onkeypress="CTtrackEnterKey(event)">
                                                    <a href="javascript:showCal('Calendar27','dd/mm/yyyy');"><img src="images/cal.gif" width="20"
                                                                                                                  height="13" border="0"
                                                                                                                  alt="Pick a date"></a><span
                                                        class="txtgrey">&nbsp;(DD/MM/YYYY)</span>
                                                </td>
                                        ...
                                        <table cellpadding=2 cellspacing=1 border=0 align="center" width="100%">
                                            <tr>
                                                <td width="100%" align="center" valign="middle">&nbsp;
                                                    <input type="button" class="myButton" value="Search" onclick="JavaScript:reopenedTicketList()"
                                                           id="buttonId0"/>
                                                </td>
                                            </tr>
                                        </table>
                ...
                <table width="100%" cellpadding=2 cellspacing=2 border=0>
                    ...
                    <%
                        ...
                        for (int iCount = 0; iCount < arlReopenedTicketList.size(); iCount += 12)
                        {
                            ...
                            if (arlReopenedTicketList.get(iCount + 3).equalsIgnoreCase(AppConstants.TASK))
                            {
                                if (arlReopenedTicketList.get(iCount + 11).equalsIgnoreCase("1"))
                                {
                    ...
                    <tr class="<%= strBackGround %>">
                        ...
                        <td class="txtgrey"><%= arlReopenedTicketList.get(iCount + 9) %>
                        </td>
                        <td class="txtgrey"><a
                                href="JavaScript:openJIRAWindow('<%= AppConstants.JIRAURL + arlReopenedTicketList.get(iCount + 1) %>', '<%= arlReopenedTicketList.get(iCount + 1) %>')"
                                class="link"><%= arlReopenedTicketList.get(iCount + 1) %>
                        </a>
                        </td>
                        <td class="txtgrey"><a
                                href="JavaScript:openJIRAWindow('<%= AppConstants.JIRAURL + arlReopenedTicketList.get(iCount + 1) %>', '<%= arlReopenedTicketList.get(iCount + 1) %>')"
                                class="link"><%= arlReopenedTicketList.get(iCount + 2) %>
                        </a>
                        </td>

                        <td class="txtgrey"><a
                                href="JavaScript:viewTicketDetails('<%= arlReopenedTicketList.get(iCount) %>','<%= arlReopenedTicketList.get(iCount + 1) %>')"
                                title="Ticket Details"><%= arlReopenedTicketList.get(iCount + 4) %>
                        </a>
                        </td>
                        <td class="txtgrey"><%= arlReopenedTicketList.get(iCount + 10) %>
                        </td>
                        <td class="<%= strSLAMissedStyle %>"><%= arlReopenedTicketList.get(iCount + 7) %>
                        </td>
                        <td class="txtgrey"><%= arlReopenedTicketList.get(iCount + 8) %>
                        </td>
                    </tr>
                    <%
                                intCounter++;
