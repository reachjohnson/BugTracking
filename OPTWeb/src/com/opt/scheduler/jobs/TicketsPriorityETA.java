...
public class TicketsPriorityETA implements Job
{
   Logger objLogger = Logger.getLogger(TicketsPriorityETA.class.getName());

   public void execute(JobExecutionContext context)
           throws JobExecutionException
   {
      objLogger.info(AppConstants.PROCESS_STARTED);
      if (AppSettings.isOpenTicketsPriority())
      {
         try (Connection objDBConnection = BaseDAO.openConnection(AppConstants.DB_JNDI))
         {
            ...
            if (AppUtil.isTodayWorkingDay(arlHolidayCalendar))
            {
               ...
               ArrayList<String> arlAssignees = new SchedulerDAO().getOpenTicketsAssigneesForTicketsPriority(objDBConnection);
               if (arlAssignees != null && !arlAssignees.isEmpty())
               {
                  ...
                  String strMailSubject = "Open Tickets to be prioritized by Bugs, Major Tasks and Ageing Tasks - As On " + AppUtil.getCurrentDate(AppConstants.JAVA_DATE_FORMAT_DD_MMM_YYYY);
                  ...
                  for (int intCount = 0; intCount < arlAssignees.size(); intCount += 2)
                  {
                     ...
                     {
                        arlTicketsPriorityETA = objSchedulerDAO.getTicketsPriorityETA(strAssignee, objDBConnection);
                        if (arlTicketsPriorityETA != null && !arlTicketsPriorityETA.isEmpty())
                        {
                           strToEmailIds = new String[]{strAssignee};
                           strMessageHeader = new String[]{strAssigneeName, "Please find the below open tickets assigned in your name.  Your priority should go by Bugs, Major Tasks and then Ageing Tasks"};
